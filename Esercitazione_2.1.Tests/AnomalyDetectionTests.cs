...
namespace Esercitazione_2._1.Tests
{
    ...
    public class AnomalyDetectionTests
    {
        ...
        [Fact]
        public void FindAnomaly_MultipleValuesAboveThreshold_ReturnsMultipleAnomalies()
        {
            var locations = new List<MonitoredLocation>
            {
                Loc(1, "N 0°0'0\"", "E 0°0'0\"", 1.5),
        ...
        [Fact]
        public void MergeAnomaliesWithNASupport_AllCoordinatesNA_SetsLinearDistanceToNA()
        {
            var locations = new List<MonitoredLocation>
            {
                Loc(1, "NA", "NA", 5.0),
        ...
        [Fact]
        public void MergeAnomaliesWithNASupport_MissingEndCoordinates_UsesNearestValidCoordinates()
        {
            var locations = new List<MonitoredLocation>
            {
                Loc(1, "N 0°0'0\"", "E 0°0'0\"", 5.0),
                Loc(2, "NA", "NA", 0.5),
                Loc(3, "N 0°0'2\"", "E 0°0'2\"", 4.4),
                Loc(4, "NA", "NA", 6.0)
            };

            var result = AnomalyDetectionService.MergeAnomaliesWithNASupport(locations, 3.0, 1);
            Assert.Equal(1, result[0].FirstId);
            Assert.Equal(4, result[0].EndId);
